...
samplename <- str_extract(all_xle,"(?<=Camsizer_raw/).*(?=/)")
analysisid <- str_extract(all_xle,"(?<=Camsizer_raw/).*(?=.xle)")
analysisid <- tstrsplit(analysisid,"/")[[2]]
...
file_process <- function(xlefile) {
  ...
  # need to skip nulls and set the file encoding because of strange format from CX2
  ...
  dat_clean <- dat_clean %>%
    ...
    mutate(cum2 = cumsum(vol_perc)) %>% # recalcualte cumulative sum as coarser than percentatge
    arrange(min_um) %>% # put back to original order
    mutate(min_um = ifelse(min_um==0,0.8,
                           min_um)) # set minimum particle size of CX2 instrument
...
# Nice colour pallete in rough coffee colours
...
ggsave("plots/grind_size_linear.pdf",grindsize, width = 8,height=6,units = "in")
ggsave("plots/beantype_linear.pdf",beantype, width = 8,height=6,units = "in")
